...
RegisterNUICallback("UpdateClothes", function(data, cb)
    local completed, retval = false, false
    if propstable[data] then
        completed, retval = ToggleProps(data)
    else
        completed, retval = ToggleClothing(data)
    end
    cb('ok')
end)
...
function ToggleClothing(whic, extra)
    ...
    if which == "tshirt" or which == "pant" or which == "bagoff" then
        extra = true
    end
        ...
        if not LastEquipped[which] then
            if Cur.Drawable ~= Table then
                PlayToggleEmote(Toggle.Emote, function()
                    ...
                    SetPedComponentVariation(PlayerPedPreview, Toggle.Drawable, Table, 0, 0)
                    if Toggle.Table.Extra then
                        ...
                        for _, v in pairs(extraToggled) do
                            ...
                            SetPedComponentVariation(PlayerPedPreview, v.Drawable, v.Id, v.Tex, 0)
            ...
            PlayToggleEmote(Toggle.Emote, function()
                SetPedComponentVariation(Ped, Toggle.Drawable, Last.Drawable, Last.Texture, 0)
                SetPedComponentVariation(PlayerPedPreview, Toggle.Drawable, Last.Drawable, Last.Texture, 0)
                LastEquipped[which] = false
                if Toggle.Table.Extra then
                    local extraToggled = Toggle.Table.Extra
                    for _, v in pairs(extraToggled) do
                        if LastEquipped[v.Name] then
                            ...
                            SetPedComponentVariation(PlayerPedPreview, Last.Id, Last.Drawable, Last.Texture, 0)
...
function ToggleProps(whic)
    ...
    if not Prop.Variants then
        if Cur.Prop ~= -1 then -- If we currently are wearing this prop, remove it and save the one we were wearing into the LastEquipped table.
            PlayToggleEmote(Prop.Emote.Off,
                function()
                    LastEquipped[which] = Cur
                    ClearPedProp(Ped, Prop.Prop); ClearPedProp(PlayerPedPreview, Prop.Prop)
                end)
            return true, false
        else
            ...
            if Last then
                PlayToggleEmote(Prop.Emote.On,
                    function()
                        SetPedPropIndex(Ped, Prop.Prop, Last.Prop, Last.Texture, true); SetPedPropIndex(PlayerPedPreview,
                            Prop.Prop, Last.Prop, Last.Texture, true)
                    end)
                LastEquipped[which] = false
                return true, true
            end
        ...
        for k, v in pairs(variations) do
            if Cur.Prop == k then
                PlayToggleEmote(Prop.Emote.On,
                    function()
                        SetPedPropIndex(Ped, Prop.Prop, v, Cur.Texture, true); SetPedPropIndex(PlayerPedPreview,
                            Prop.Prop, v,
                            Cur.Texture, true)
                    end)
                return true, true
...
CreateThread(function()
    ...
    addNewVariation("jackets", "male", 19, 20)
