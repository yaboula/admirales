...
QBCore.Functions.CreateCallback('qb-weapons:server:RepairWeapon', function(source, cb, RepairPoint, data)
    ...
    if not exports['codem-inventory']:RemoveItem(src, data.name, 1, data.slot, 'qb-weapons:server:RepairWeapon') then
...
QBCore.Functions.CreateCallback('prison:server:checkThrowable', function(source, cb, weapon)
    ...
    for _, v in pairs(Config.Throwables) do
        if QBCore.Shared.Weapons[weapon].name == 'weapon_' .. v then
            if not exports['codem-inventory']:RemoveItem(source, 'weapon_' .. v, 1, false, 'prison:server:checkThrowable') then return cb(false) end
            throwable = true
            break
        end
    end
...
RegisterNetEvent('qb-weapons:server:TakeBackWeapon', function(k)
    ...
    exports['codem-inventory']:AddItem(src, itemdata.name, 1, false, itemdata.info, 'qb-weapons:server:TakeBackWeapon')
...
RegisterNetEvent('qb-weapons:server:removeWeaponAmmoItem', function(item)
    ...
    exports['codem-inventory']:RemoveItem(source, item.name, 1, item.slot, 'qb-weapons:server:removeWeaponAmmoItem')
end)
...
local function EquipWeaponTint(source, tintIndex, item, isMK2)
    ...
    Player.Functions.SetInventory(Player.PlayerData.items, true)
    exports['codem-inventory']:RemoveItem(source, item, 1, false, 'qb-weapon:EquipWeaponTint')
...
local function EquipWeaponAttachment(src, item)
    ...
    if shouldRemove then
        exports['codem-inventory']:RemoveItem(src, item, 1, false, 'qb-weapons:EquipWeaponAttachment')
    ...
    if Inventory[WeaponData.slot] then
        if Inventory[WeaponData.slot].info.attachments and next(Inventory[WeaponData.slot].info.attachments) then
            ...
            if HasAttach then
                ...
                Player.Functions.SetInventory(Player.PlayerData.items, true)
                exports['codem-inventory']:AddItem(src, AttachmentData.attachment, 1, false, false, 'qb-weapons:server:RemoveAttachment')
