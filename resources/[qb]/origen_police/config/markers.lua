-- [INST INST-028] Mapas activos: cada valor es el nombre del JSON en
-- `config/police-stations/` **sin** `.json` (ej. `sherif.json` → `"sherif"`).
-- Puedes tener varias comisarías a la vez: [1], [2], …
-- JSON incluidos en el pack: default-gta, mission-row, breze-mrpd, artex-mrpd,
-- nopixel-mrpd, sherif (custom), custom, etc.
Config.Maps = {
    [1] = "default-gta",
    [2] = "sherif",
}

Config.MarkersDraw = true
MarkersList = {
...
function LoadMarkers()
    -- Reset acumuladores antes de volver a leer los JSON (evita duplicar
    -- Public.Markers / BillsNPC si LoadMarkers corre más de una vez; y permite
    -- que ReloadMarkers funcione con Config.IgnoreSettings = true).
    Tables.Markers = {}
    Public.Markers = {}
    BillsNPCPositions = {}
    SpawnConfiscatedVehicles = {}
    MapOptions = {}
    if ClearNPCsBills then
        ClearNPCsBills()
    end
    ...
    for mapIndex, mapName in pairs(Config.Maps) do
        ...
        if type(mapData) ~= "table" then
            print(("^1[Origen Police]^0 LoadMarkers: mapa %q no cargó (JSON ausente o inválido). Revisa config/police-stations/%s.json"):format(tostring(mapName), tostring(mapName)))
            MapOptions[mapIndex] = {}
            goto skip_bad_map
        end
...
function ReloadMarkers()
    ...
    -- Antes: `if Config.IgnoreSettings then return end` impedía recargar cuando
    -- había mapas en markers.lua — el tablet llamaba ReloadMarkers en vano y
    -- tras editar JSON / Config.Maps no se aplicaba nada sin truco.
    LoadMarkers()
