...
function HasPermissionMenu(permName){
	// [INST INST-028 DIAG] Diagnóstico temporal — 2026-04-23
	try {
		var _pjob = permissions[jobData && jobData.name];
		var _need = _pjob ? _pjob[permName] : undefined;
		var _lvl  = jobData && jobData.level;
		var _blk  = (typeof _lvl === 'number' && typeof _need === 'number') ? (_lvl < _need) : undefined;
		sendNotification(
			_blk ? 'error' : 'success',
			'DIAG ' + permName,
			'job=' + (jobData && jobData.name) + ' lvl=' + _lvl + ' need=' + _need + ' blocked=' + _blk
		);
	} catch(e) { sendNotification('error', 'DIAG EXC', String(e)); }
...
window.addEventListener('message', function (event) {
	if (event.data.action == 'alpr') {
		...
		if(event.data.options.holster != true) {
			$(".police-tab[tab='guns']").hide();
		}
