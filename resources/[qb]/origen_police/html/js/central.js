centralFunctions = {
	...
	loadCentralEvents: () => {
		$(document).on('click', '.police .btn-police-central', function () {
			...
			sendNotification('success', 'DIAG click', 'btn-police-central (Dispatch)');
			...
			// [INST INST-028 DIAG]
			sendNotification(valid ? 'error' : 'success', 'DIAG central tab', valid ? 'tab Dispatch YA existe → click no hace nada' : 'tab no existe → abriendo...');
			if (!valid) {
				fetch('LoadPolicePage', { page: 'central' }).done((cb) => {
					// [INST INST-028 DIAG]
					sendNotification(cb ? 'success' : 'error', 'DIAG central cb', 'cb=' + JSON.stringify(cb).slice(0,80));
					if (cb) {
