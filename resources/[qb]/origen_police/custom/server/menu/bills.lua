...
function GetDebors(jobCategory)
    ...
    if Config.Framework == "qbcore" then
        ...
        result = MySQL.awaitQuery('SELECT opb.citizenid, SUM(opb.price) AS totalprice, p.charinfo, p.image FROM origen_police_bills opb INNER JOIN players p ON opb.citizenid = p.citizenid WHERE opb.payed = 0 AND opb.job = ? GROUP BY opb.citizenid', {jobCategory})

    else

        result = MySQL.awaitQuery('SELECT opb.citizenid, SUM(opb.price) AS totalprice, u.firstname, u.lastname, u.image FROM origen_police_bills opb INNER JOIN users u ON opb.citizenid = u.identifier WHERE opb.payed = 0 AND opb.job = ? GROUP BY opb.citizenid', {jobCategory})
